...
## Распределение ролей, архитектура и  структура классов

> Этот файл — единый источник правды для команды из 4 человек, который будет помогать конкретному участнику. Каждый раздел закреплён за одной ролью. Не меняйте контракты (сигнатуры методов, имена классов/полей) в одиночку — любое изменение контракта согласуется в общем чате/PR, иначе части проекта перестанут собираться вместе.
